...
To sync with the updates from Quartz:

```sh
$ git remote -v
origin	https://github.com/eulersson/notes.git (fetch)
origin	https://github.com/eulersson/notes.git (push)
...
- **v4** simply tracks the remote.
- **custom**: branches off v4, has our customization layer, but nothing about the content to publish.
- **main**: branches off custom, has commits only that have to do with published content.
...
- Contribute to quartz to resolve properly header links within the same page.
- Prompt for text message to show in commit, if empty, default to what we have already.
- Table of Contents broken when Tibetan text because it wraps it with the span to make it larger, I should fix my plugin to avoid that.